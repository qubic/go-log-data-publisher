...
func (m mockTickStore) AddProcessed(_ domain.LogEvent) {
}

func (m mockTickStore) AddSkipped(_ domain.LogEvent) {
}

func (m mockTickStore) UpdateTickHeight(_ context.Context) error {
	return nil
}
...
func TestConsumeBatch_Success(t *testing.T) {
	...
	consumer := NewConsumer(mockKafka, mockElastic, &mockTickStore{}, m, map[uint64][]int16{0: {0, 1, 2, 3, 8, 13}})
	...
	if consumer.highestTick != 1000 {
		t.Errorf("Expected currentTick to be 1000, got: %d", consumer.highestTick)
	}
...
func TestConsumeBatch_MultipleRecords(t *testing.T) {
	...
	if consumer.highestTick != 1001 {
		t.Errorf("Expected currentTick to be 1001 (highest), got: %d", consumer.highestTick)
	}
}
...
func TestConsumeBatch_filterIfLogIsNotSupported(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			consumer := NewConsumer(mockKafka, mockElastic, &mockTickStore{}, m, map[uint64][]int16{0: {0, 1, 2, 3, 4, 5, 6, 8, 13, 255}})
