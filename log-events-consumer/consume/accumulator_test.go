...
func TestAccumulator(t *testing.T) {
	acc := NewAccumulator()
	// 1. Add processed log
	acc.AddProcessed(domain.LogEvent{TickNumber: 100, Index: 1})
	// 2. Add skipped log
	acc.AddSkipped(domain.LogEvent{TickNumber: 100, Index: 2})
	// 3. Set total for tick 100
	acc.AddProcessed(domain.LogEvent{TickNumber: 100, Index: 3, LastLogForTick: true})
	// 4. Add for another tick
	acc.AddProcessed(domain.LogEvent{TickNumber: 101, Index: 1})

	snapshot := acc.Drain()
	require.Len(t, snapshot, 2)

	// 5. Ensure Drain cleared the data and doesn't influence snapshot
	acc.AddProcessed(domain.LogEvent{TickNumber: 102, Index: 1})
